...
private[spark] class ComputeScheduler(
  ...
  private def pollForMonotask(): Option[ComputeMonotask] = synchronized {
    ...
    if (freeMemory > 0) {
      ...
      val monotaskFromQuarantine = quarantineQueue.peekFirst()
	  val monotaskFromNonQuarantine = monotaskQueue.peekFirst()
      if (monotaskFromQuarantine != null) {
		if (monotaskFromQuarantine.submitTime < monotaskFromNonQuarantine.submitTime) {
			val monotaskQ = quarantineQueue.poll()
			Some(monotaskQ)
		} else {
